...
namespace AMS.CIM.Caojin.RPTLibrary.Models
{
    public class ReqRpt025Translator
    {
        ...
        DB2DataPusher<RPT_Move_By_EqpType> MovePusher = new DB2DataPusher<RPT_Move_By_EqpType>("ISTRPT.RPT_Move_By_EqpType");

        List<RPT_RealTime_Lin> RealTimeList = new List<RPT_RealTime_Lin>();

        List<RPT_Turn_Daily> TurnDailyList = new List<RPT_Turn_Daily>();

        List<RPT_Move_By_EqpType> MoveList = new List<RPT_Move_By_EqpType>();
        ...
        private void DoWork()
        {
            ...
            while (firstTime < End_Time)
            {
                ...
                if (rawList.Count() > 0)
                {
                    ...
                    rawList.Select(s => new { s.Claim_Time, s.Cur_Wafer_Qty, EqpType = s.Eqp_ID.Substring(0, 6), s.ProdSpec_ID }).GroupBy(g => new { g.ProdSpec_ID, g.EqpType }).Select(s => new { Product = s.Key, Qty = s.Sum(i => i.Cur_Wafer_Qty) }).ToList().ForEach(f=>MoveList.Add(new RPT_Move_By_EqpType(){ Product_ID=f.Product.ProdSpec_ID,EqpType=f.Product.EqpType,Start_Time=firstTime,MoveQty=f.Qty}));
                }
                if (rawList2.Count() > 0)
                {
            ...
            var MoveCurrent = new DB2DataCatcher<RPT_Move_By_EqpType>("ISTRPT.RPT_Move_By_EqpType") { Conditions = string.Format("where Start_Time >='{0}' and Start_Time<='{1}'", Start_Time.ToString("yyyy-MM-dd HH:mm:ss"), End_Time.ToString("yyyy-MM-dd HH:mm:ss")) }.GetEntities().EntityList;
            ...
            if (MovePusher.entities.EntityList.Count() > 0) MovePusher.PushEntities();
