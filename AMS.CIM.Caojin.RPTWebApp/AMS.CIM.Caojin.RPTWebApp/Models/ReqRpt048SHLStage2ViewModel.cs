    ...
    public class ReqRpt048SHLStage2ViewModel
    {
        ...
        private void SetQtValue()
        {
            ...
            foreach (var qt in QtCatcher.entities.EntityList)
            {
                ...
                {
                    qtime = qt.Duration / (list.Last().StepComplete.Value-list.First().WFIn.Value).TotalMinutes;
                    type = "h";
                }
                //case Future
                else if (qt.Oper_NO.CompareTo(curFlow.Ope_NO)> 0)
                {
                    qtime = qt.Duration / list.Sum(s => s.CT);
                    type = "f";
                }
                //case current
                else
                {
                    ...
                    double remainCt = list.SkipWhile(s => s.OpeNO != curFlow.Ope_NO).Sum(s => s.CT);
                    qtime = (totalQt-totalUsed) / remainCt;
                    type = "c";
                }
