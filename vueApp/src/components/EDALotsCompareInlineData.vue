    ...
    <BaseContainer>
      ...
      <template slot="main">
        <BaseTableContainer
          ...
          fileName="EDA_LotsCompareInlineData.xls"
        >
                ...
                <tr v-for="(row,idx) in scope.datas" :key="idx">
                  ...
                  <td
                    ...
                    v-text="row.WaferValue[wafer]"
                     :class="(row.WaferValue[wafer]<row.LC&&row.LC!==null)||(row.WaferValue[wafer]>row.UC&&row.UC!==null)?'eda-red':''"
                  ></td>
                  <td v-text="row.LS"></td>
                  <td v-text="row.LC"></td>
                  <td v-text="row.Target"></td>
...
export default {
  ...
  computed: {
    ...
    prodFilteredData: function() {
      ...
      if (this.prod !== null && this.prod !== "") {
        ary = ary.filter(f => f.ProdID.toLowerCase().indexOf( this.prod.toLowerCase())===0);
      }
      return ary;
    },
  ...
  methods: {
    ...
    queryFun(data, cb) {
      let url = this.URL_PREFIX + "/ReqRpt206/ManualQuery";
      let _this = this;
      this.loading = true;
      this.$http
        .post(url, data)
    ...
    querySearch(queryString, cb) {
      ...
      let results = queryString
        ? avaProds.filter(f=>f.ProdSpec_ID.toLowerCase().indexOf(
