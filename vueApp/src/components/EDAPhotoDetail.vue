      ...
      <template slot="header">
        <el-row type="flex" justify="center">
          ...
          <el-col :lg="4" :offset="2" :md="8" :sm="12">
            <BaseHeaderCard project="RPT000214" coder="戴新亚（0607）" user="姜兆涛（0102）"></BaseHeaderCard>
          </el-col>
        </el-row>
      </template>
      <template slot="main">
...
export default {
  ...
  computed: {
    allRoutes: function() {
      let data=this.autoTableData.map(m => m.RouteID)
      //let data=this.autoTableData.map(m=>({RouteID: m,len: m.length}))//.map(m=>m.RouteID).sort((a,b)=>a.len-b.len)
    ...
    prodFilteredData: function() {
      ...
      if (this.route !== null && this.route !== "") {
        ary = ary.filter(f => f.RouteID === this.route);
      }
      return ary;
    },
    ...
    allLotId: function() {
      let data = this.operNameFilteredData.map(m => m.LotID);
      data.distinct();
      data.sort();
      this.lotID = [];
      return data;
  ...
  methods: {
    ...
    queryFun(data, cb) {
      let url = this.URL_PREFIX + "/ReqRpt214/ManualQuery";
      let _this = this;
      this.loading = true;
      this.$http
        .post(url, data)
